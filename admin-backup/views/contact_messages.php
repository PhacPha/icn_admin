...
<h1 class="text-3xl font-bold text-black mb-6 animate__animated animate__fadeIn">ข้อความติดต่อจากลูกค้า</h1>
                    ...
                    <tbody id="messages-table">
                        <?php foreach ($GLOBALS['contact_messages'] as $message): ?>
                            <tr class="bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200" data-id="<?php echo $message['id']; ?>">
                                ...
                                <td class="p-3 message-preview" onclick="showMessageDetail(<?php echo $message['id']; ?>, '<?php echo htmlspecialchars($message['name']); ?>', '<?php echo htmlspecialchars($message['email']); ?>', '<?php echo htmlspecialchars($message['subject']); ?>', '<?php echo htmlspecialchars(addslashes($message['message'])); ?>', '<?php echo htmlspecialchars($message['submitted_at']); ?>', <?php echo $message['is_read']; ?>)">
                                    <?php
    ...
    <div id="message-detail-modal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 hidden">
        <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-lg">
            ...
            <div class="flex gap-2 mt-4">
                <button onclick="hideMessageDetail()" class="bg-gray-500 text-white p-2 rounded-lg hover:bg-gray-600 transition-colors duration-200 flex-1">ปิด</button>
            </div>
        </div>
    </div>
